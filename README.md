...
**Status:** Slices **S1–S3** delivered. Application foundation, PostgreSQL 16 with the required
extensions, health endpoints, the English interface shell, CI guard rails, authentication with four
roles behind a single backend authorization choke point, an append-only audit trail, and the
admin-managed Specification Dictionary with its accessible information popover. The RF domain model
begins at S4; see the [slice plan](docs/design/05-vertical-slice-plan.md) for what lands when.
